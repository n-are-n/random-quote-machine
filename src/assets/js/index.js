...
let tumblr = document.getElementById('tumblr-quote')
let tweetURL = 'https://www.twitter.com/intent/tweet?hashtags=quotes&related=freecodecamp&text="'
let tumblrURL = 'https://www.tumblr.com/widgets/share/tool?posttype=quote&tags=quotes,freecodecamp&caption='
let color = ['red', 'blue', 'green', 'yellow', 'pink']
let generateRandomNumber = (num) => Math.floor(Math.random() * num)
let updateElementContent = (elements, contents) => elements.forEach((element, index) => element.innerText = contents[index])
let applyAnimation = (...elements) => elements.forEach(element => element.classList.toggle('animate')) 
function getData() {
    try
    {
        GET().then(Response => {
